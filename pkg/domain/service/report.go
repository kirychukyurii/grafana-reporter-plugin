...
func NewReportService(settings *config.ReporterAppConfig,
	grafanaHTTPClient grafana.GrafanaHTTPAdapter,
...
func (r *Report) exportCSV(ctx context.Context, dashboard *gutils.Dashboard, panel gutils.Panel, tmpDir string) error {
	...
	url := fmt.Sprintf("%s/d/%s/db?orgId=1&inspect=%d&inspectTab=data", r.settings.GrafanaConfig.URL, dashboard.Model.Uid, panel.Id)
	headers := []string{"Authorization", r.settings.GrafanaConfig.BasicAuth()}
	if err = page.Prepare(url, headers, nil); err != nil {
		return err
	}
